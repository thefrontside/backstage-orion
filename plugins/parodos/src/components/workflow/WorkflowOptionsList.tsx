...
export function WorkflowOptionsList({ project, workflowOptions }: WorkflowOptionsListProps): JSX.Element {
      ...
      <Grid container direction="row" spacing={2}>
        {workflowOptions.map(workflowOption => (
          <Grid item xs={12} lg={6} xl={4} key={workflowOption.identifier}>
            <Card className={styles.applicationCard} variant="elevation" elevation={3}>
              <CardMedia>
                <CardContent>{workflowOption.type}</CardContent>
                <ItemCardHeader title={workflowOption.displayName} classes={{ root: styles.applicationHeader }} />
              </CardMedia>
