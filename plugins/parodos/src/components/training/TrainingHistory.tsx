...
export const TrainingHistory = () => {
  ...
  const onFilterProjects = (arg: SelectedItems) => {
    setFilteredTrainings(
      arg === 'All Courses' ? trainings : trainings.filter(training => training.course.title === arg),
    );
  };
  ...
  return (
    <Grid container direction="column" spacing={2}>
      <Grid item xs={3}>
        <Select onChange={onFilterProjects} placeholder="All Courses" label="Filter by" items={projectFilterItems} />
      </Grid>
      <Grid item>
